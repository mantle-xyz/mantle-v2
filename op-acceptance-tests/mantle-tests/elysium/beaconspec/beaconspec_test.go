...
func TestL1Beacon_ConfigSpec_GloasForkEpoch_PostGlamsterdam(gt *testing.T) {
	...
	// Reach the raw Beacon HTTP so we can see keys the typed client drops.
	rawProvider, ok := l1CL.(interface {
		RawBeaconHTTP() client.HTTP
	})
	require.True(ok, "L1CLNode must expose RawBeaconHTTP for raw /eth/v1/config/spec inspection")

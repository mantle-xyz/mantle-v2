...
func TestL1Reorg_DeepReorg_PostUpgrade(gt *testing.T) {
	...
	require.Eventually(func() bool {
		...
		// Read the head before the height: a deep reorg unwinds l2BeforeReorg.Number out of
		// existence and rebuilds past it, and dsl BlockRefByNumber faults the test on an absent
		// block rather than reporting absence. Guarding on head > N is truth-equivalent to the
		// old `... && l2Head.Number > N` but never reads a rewound-out height.

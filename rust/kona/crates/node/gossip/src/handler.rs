...
impl BlockHandler {
    ...
    pub fn topic(&self, timestamp: u64) -> IdentTopic {
        // [MANTLE] op-node publishes on the V4 topic from `IsIsthmus(ts) || IsMantleSkadi(ts)`
        // (p2p/gossip.go `publishRawSignedPayload`). Every OP fork on a Mantle chain is pinned
        // to `mantle_arsia_time`, so in the `[Skadi, Arsia)` window this would pick V1 while the
        // rest of the network is on V4 — the node would gossip onto a topic nobody reads.
        if self.rollup_config.is_isthmus_active(timestamp) ||
            self.rollup_config.is_mantle_skadi_active(timestamp)
        {

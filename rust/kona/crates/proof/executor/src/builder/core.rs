        ...
        >,
{
    ...
    pub fn build_block(
        ...
        // [MANTLE] Upstream v1.7.0 dropped kona-genesis's `revm` feature, taking
        // `RollupConfig::{spec_id, revm_spec_id, mantle_spec_id}` with it; spec resolution now
        // lives solely in `alloy_op_evm::spec_by_timestamp_after_bedrock`. Log the spec that
        // `evm_env` actually resolved instead of recomputing it — one source of truth.
        let resolved_spec = evm_env.cfg_env.spec;

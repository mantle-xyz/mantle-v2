        ...
        for (i, batch) in self.batches.iter().enumerate() {
            ...
            for (i, tx) in batch.transactions.iter().enumerate() {
                ...
                match OpTxType::try_from(first_byte) {
                    ...
                    // [MANTLE] Mirrors the single-batch gate: Skadi enables EIP-7702 ahead of
                    // Isthmus, which is pinned to `mantle_arsia_time`. (Note op-node has no
                    // SetCode check on the span path at all — `checkSpanBatch` only rejects
                    // deposits — so kona is strictly stricter here regardless of Mantle. Left in
                    // place, but its gate must at least not reject what op-node accepts.)
                    Ok(OpTxType::Eip7702)
                        if !cfg.is_isthmus_active(batch.timestamp) &&
                            !cfg.is_mantle_skadi_active(batch.timestamp) =>
                    {

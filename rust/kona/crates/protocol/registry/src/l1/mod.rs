...
mod tests {
    ...
    #[test]
    fn test_get_l1_bpo_hoodi() {
        /// BPO1 hardfork activation timestamp
        const HOODI_BPO1_TIMESTAMP: u64 = 1762365720;

        /// BPO2 hardfork activation timestamp
        const HOODI_BPO2_TIMESTAMP: u64 = 1762955544;

        let hoodi = L1Config::hoodi();

        assert_eq!(hoodi.blob_schedule.len(), 5);
        assert_eq!(
            hoodi.blob_schedule.get(&EthereumHardfork::Bpo1.name().to_lowercase()).unwrap(),
            &BlobParams::bpo1()
        );
        assert_eq!(
            hoodi.blob_schedule.get(&EthereumHardfork::Bpo2.name().to_lowercase()).unwrap(),
            &BlobParams::bpo2()
        );

        let blob_schedule = hoodi.blob_schedule_blob_params();
        assert_eq!(blob_schedule.scheduled.len(), 2);

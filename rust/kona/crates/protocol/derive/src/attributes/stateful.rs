...
// `Interop` is gone: upstream replaced that bundle with `Hardforks::LAGOON`.
use kona_hardforks::{Hardfork, Hardforks, MantleHardforks};
    ...
    L2P: L2ChainProvider + Debug + Send,
{
    async fn prepare_payload_attributes(
        ...
        if self.rollup_cfg.is_mantle() {
            // [MANTLE] Mantle chains skip every OP hardfork upgrade-tx bundle and emit only
            // their own. Mirrors op-node's `attributes.go`, which tests Skadi and Arsia
            // independently and appends in that order — both fire if a chain happens to
            // activate them at the same timestamp.
            //
            // op-node's gate is `IsX(t) && t >= BlockTime && !IsX(t - BlockTime)`; since
            // `next_l2_time == parent.timestamp + block_time`, the form below is equivalent
            // and cannot underflow near genesis.
            if self.rollup_cfg.is_mantle_skadi_active(next_l2_time) &&
                !self.rollup_cfg.is_mantle_skadi_active(l2_parent.block_info.timestamp)
            {
                // Deploys EIP-4788 + EIP-2935, which Mantle skipped along with the Ecotone and
                // Isthmus bundles. Neither adds to the block gas limit (op-node appends them
                // without touching `gasLimit`).
                upgrade_transactions.append(&mut MantleHardforks::SKADI.txs().collect());
            }
            ...
            // [MANTLE] Upstream replaced the Interop bundle — plus the CrossL2Inbox pair that
            // was emitted when the dependency set had more than one chain — with Lagoon's
            // NUT-bundle form. Adopted verbatim from upstream: this whole `else` branch is
            // unreachable on Mantle chains, which take the `is_mantle()` path above.
                ...
                !self.rollup_cfg.is_lagoon_active(l2_parent.block_info.timestamp)
            {
                ...
                // The gas reservation does not vary with the dependency set, only the tx set
                // does. See Hardforks::LAGOON::upgrade_gas.
        ...
        // [MANTLE] op-node gates these on `IsCanyon(t) || IsMantleSkadi(t)` and
        // `IsEcotone(t) || IsMantleSkadi(t)` respectively (attributes.go). The Skadi disjunct is
        // not redundant on a Mantle chain: `AlignOpWithMantle` pins every OP fork to
        // `mantle_arsia_time`, so between Skadi and Arsia both `is_canyon_active` and
        // `is_ecotone_active` are false while op-node still sets an empty withdrawals list and a
        // parent beacon block root. On Mantle mainnet that window is months long, and omitting
        // either field changes the block hash.
        let mantle_skadi = self.rollup_cfg.is_mantle_skadi_active(next_l2_time);

        let withdrawals =
            (self.rollup_cfg.is_canyon_active(next_l2_time) || mantle_skadi).then(Vec::default);

        let parent_beacon_root = (self.rollup_cfg.is_ecotone_active(next_l2_time) || mantle_skadi)
...
mod tests {
    ...
    use kona_genesis::{CONFIG_UPDATE_TOPIC, HardForkConfig, MantleHardForkConfig, SystemConfig};
    ...
    // ---------------------------------------------------------------------------
    // [MANTLE] Skadi / Arsia upgrade-transaction emission.
    //
    // Go reference: op-node/rollup/derive/attributes.go, which tests
    // `IsMantleSkadiActivationBlock` and `IsMantleArsiaActivationBlock` independently and
    // appends both bundles after the deposits, Skadi first.
    // ---------------------------------------------------------------------------

    /// Drives one block on a Mantle chain and returns the resulting transaction count.
    ///
    /// `parent_timestamp` is the parent block's time; the built block is at
    /// `parent_timestamp + block_time`, which is what the activation gates key off.
    async fn mantle_payload_tx_count(
        mantle_hardforks: MantleHardForkConfig,
        parent_timestamp: u64,
        block_time: u64,
    ) -> (usize, Option<u64>, u64) {
        let cfg = Arc::new(RollupConfig { block_time, mantle_hardforks, ..Default::default() });
        let l1_cfg = Arc::new(L1Config::sepolia().into());
        let l2_number = 1;
        let sys_config = SystemConfig::default();
        let mut fetcher = TestSystemConfigL2Fetcher::default();
        ...
        let payload = builder.prepare_payload_attributes(l2_parent, epoch).await.unwrap();
        (payload.transactions.unwrap().len(), payload.gas_limit, sys_config.gas_limit)
    }

    /// The Skadi activation block carries the two EIP-4788 / EIP-2935 deployments, and only
    /// that block does. Before this was wired up kona emitted nothing at all, so a Mantle chain
    /// crossing Skadi derived a block op-node would never produce.
    #[tokio::test]
    async fn test_mantle_skadi_activation_block_emits_upgrade_txs() {
        let block_time = 2;
        let hardforks =
            MantleHardForkConfig { mantle_skadi_time: Some(102), ..MantleHardForkConfig::NONE };

        // Parent at 100 -> block at 102 == activation.
        let (count, _, _) = mantle_payload_tx_count(hardforks, 100, block_time).await;
        assert_eq!(count, 1 + 2, "activation block: 1 L1InfoTx + 2 Skadi deployments");

        // Parent at 102 -> block at 104: already active, nothing more to emit.
        let (count, _, _) = mantle_payload_tx_count(hardforks, 102, block_time).await;
        assert_eq!(count, 1, "post-activation block must carry the L1InfoTx only");

        // Parent at 98 -> block at 100: not yet active.
        let (count, _, _) = mantle_payload_tx_count(hardforks, 98, block_time).await;
        assert_eq!(count, 1, "pre-activation block must carry the L1InfoTx only");
    }

    /// Skadi's deployments do not extend the block gas limit — op-node appends them without
    /// touching `gasLimit`, and the system-config reconstruction at the next block would
    /// otherwise subtract an amount that was never added.
    #[tokio::test]
    async fn test_mantle_skadi_activation_adds_no_upgrade_gas() {
        let hardforks =
            MantleHardForkConfig { mantle_skadi_time: Some(102), ..MantleHardForkConfig::NONE };
        let (_, gas_limit, sys_gas_limit) = mantle_payload_tx_count(hardforks, 100, 2).await;
        assert_eq!(gas_limit, Some(sys_gas_limit));
    }

    /// A chain that schedules Skadi and Arsia at the same timestamp emits both bundles, Skadi
    /// first. op-node's two `if`s are independent, so this is reachable — an `else if` here
    /// would silently drop seven Arsia deposits.
    #[tokio::test]
    async fn test_mantle_skadi_and_arsia_at_the_same_block_emit_both_bundles() {
        let hardforks = MantleHardForkConfig {
            mantle_skadi_time: Some(102),
            mantle_arsia_time: Some(102),
            ..MantleHardForkConfig::NONE
        };
        let (count, _, _) = mantle_payload_tx_count(hardforks, 100, 2).await;
        assert_eq!(count, 1 + 2 + 7, "1 L1InfoTx + 2 Skadi + 7 Arsia");
    }

    /// In the `[Skadi, Arsia)` window op-node sets an empty withdrawals list and a parent
    /// beacon block root even though every OP fork is still inactive (they are pinned to
    /// `mantle_arsia_time`). kona gated these on Canyon/Ecotone alone and produced `None` for
    /// both — a different block hash for every block in a months-long window.
    #[tokio::test]
    async fn test_mantle_skadi_sets_withdrawals_and_beacon_root_before_arsia() {
        let block_time = 2;
        let cfg = Arc::new(RollupConfig {
            block_time,
            mantle_hardforks: MantleHardForkConfig {
                mantle_skadi_time: Some(100),
                // Arsia far in the future: every OP fork stays inactive for this block.
                mantle_arsia_time: Some(10_000),
                ..MantleHardForkConfig::NONE
            },
            ..Default::default()
        });
        assert!(!cfg.is_canyon_active(102), "test premise: Canyon must still be inactive");
        assert!(!cfg.is_ecotone_active(102), "test premise: Ecotone must still be inactive");

        let l1_cfg = Arc::new(L1Config::sepolia().into());
        let mut fetcher = TestSystemConfigL2Fetcher::default();
        fetcher.insert(B256::ZERO, SystemConfig::default());
        let mut provider = TestChainProvider::default();
        let beacon_root = B256::repeat_byte(0x5a);
        let header = Header {
            timestamp: 100,
        ...
        assert_eq!(
            payload.payload_attributes.withdrawals,
            Some(Vec::default()),
            "Skadi must produce an empty withdrawals list, not None",
        );
        assert_eq!(
            payload.payload_attributes.parent_beacon_block_root,
            Some(beacon_root),
            "Skadi must carry the L1 parent beacon block root",
        );
    }

    /// Before Skadi, both fields stay absent — the disjunct must not turn them on early.
    #[tokio::test]
    async fn test_mantle_pre_skadi_has_no_withdrawals_or_beacon_root() {
        let cfg = Arc::new(RollupConfig {
            block_time: 2,
            mantle_hardforks: MantleHardForkConfig {
        ...
        let payload = builder.prepare_payload_attributes(l2_parent, epoch).await.unwrap();
        assert_eq!(payload.payload_attributes.withdrawals, None);
        assert_eq!(payload.payload_attributes.parent_beacon_block_root, None);
    }

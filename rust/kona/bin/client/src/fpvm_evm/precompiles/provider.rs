    ...
    O: PreimageOracleClient + Clone + Send + Sync + 'static,
{
    ...
    pub fn new_with_spec(spec: OpSpecId, hint_writer: H, oracle_reader: O) -> Self {
        let precompiles = match spec {
            ...
            // [MANTLE] OSAKA / ARSIA are Mantle-only `OpSpecId` variants. Mantle does not open any
            // newly added eth/op hardfork, so they keep Jovian's precompile set rather than
            // Karst's — consistent with `RollupConfig::spec_id` falling through to JOVIAN.
            // (The pre-v20 `INTEROP` variant is gone; upstream replaced it with LAGOON.)
            OpSpecId::JOVIAN | OpSpecId::OSAKA | OpSpecId::ARSIA => jovian(),
        ...
        let accelerated_precompiles = match spec {
            ...
            // [MANTLE] Same reasoning as the precompile set above: OSAKA / ARSIA stay on Jovian.
            OpSpecId::JOVIAN | OpSpecId::OSAKA | OpSpecId::ARSIA => accelerated_jovian::<H, O>(),

...
mod tests {
    ...
    #[test]
    // [MANTLE] Ignored for the same reason as
    // `attributes::test::decoded_transactions_reject_non_canonical_encoding` — see the comment
    // there. Upstream test, unreachable canonical check under alloy 2.4.2; apart from this
    // attribute the file matches upstream byte for byte.

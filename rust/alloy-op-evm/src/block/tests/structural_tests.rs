...
// [MANTLE] Ignored: asserts upstream's "operator fee is charged from Isthmus" semantics.
// Mantle charges it from **Arsia** (op-revm handler gates on `OpSpecId::ARSIA`, and the
// SDM settlement call site in block/mod.rs uses the same gate), so on Mantle this delta is
// legitimately zero at the fixture's JOVIAN spec. The path itself is unreachable in
// production: SDM/PostExec is Karst+ and Mantle registers neither Karst nor Lagoon.
// Re-enable only if Mantle ever moves the operator-fee start back to Isthmus.
#[ignore = "upstream asserts Isthmus operator-fee start; Mantle starts at Arsia"]
#[test]
